...
const articles_list_endpoint = '/api/events?populate=*';
...
export default function MainContentEvents() {


    const { data : dataEvent, error, isLoading } = useSWR<StrapiResponse<ArticleListItem[]>>(`${NEXT_URL_BACKOFFICE}${articles_list_endpoint}`, fetcher)
    ...
    console.log('Articles List SWR: ',dataEvent);

    return(
        <>
            <div className="hidden col-span-8 md:block relative z-50 ">
                <div className={'grid grid-cols-1 md:grid-cols-3 gap-8'}>
                    {dataEvent.data.map((item: ArticleListItem) => (
                        <div key={item.id} className={'h-full'}>
                            {/*<CardComponent />*/}
                            <CardComponent
                                title={item.title}
                                url={'/blog/events/'+item.slug}
